<%@ Page Language="C#" AutoEventWireup="false" CodeBehind="RegisterEx1.aspx.cs" Inherits="PCIWebFinAid.RegisterEx1" ValidateRequest="false" %>
...
<table style="width:99%">
	<tr id="trCCNumber">
		<td style="white-space:nowrap">
			...
			<span id="txIFrameCC" style="width:300px"></span>
...
<!-- HiddenField run@t="server" ID="txConcatenatedString" -->
...
function TokenFinish()
{
	...
	if ( txFrame != null )
		try
		{
//			txFrame.validate(); alert(JSON.stringify(txCC));
//			alert(txCC.isValid); alert(txCC.isCvvValid);
			if ( txCC.isValid && txCC.isCvvValid )
			{
	//			var k = 0;
	//			err   = 'TokenEx failed to return tokens for card number and/or CVV'; alert('3');
				txFrame.tokenize();
//				while ( Trim(txEncrypt).length == 0 )
//					k++;
//				alert("txEncrypt='"+txEncrypt+"', k="+k.toString());
				return true;
			}
		}
		catch (x)
...
//		cvv: true,
//		cvvContainerID: "txtCCCVV",

	try
	{
		txFrame = new TokenEx.Iframe("txIFrameCC", txConfig);
//		txFrame.on("load", function() { txCC = null; });
...
//		txFrame.on("validate", function (data) { alert('validate, data='+JSON.stringify(data)); txCC = data; });
...
//		txFrame.on("cardTypeChange", function (data) { util.log("Iframe cardTypeChange:" + JSON.stringify(data)); });
		txFrame.on("tokenize", function (data) { if (data.token) { SetEltValue('txToken',data.token);
																					  frmRegister.submit(); } });
//		txFrame.on("tokenize", function (data) { txEncrypt = data.token; SetEltValue('txToken',txEncrypt); });
		txFrame.on("error", function (data) { alert('error, data='+JSON.stringify(data)); util.log("Iframe error:" + JSON.stringify(data)); }); //major error occured
		txFrame.load();
	}
	catch (ex)
	{
